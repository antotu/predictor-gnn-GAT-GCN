      ...
      for numUnit2 in [u for u in (numUnit1>>i for i in range(10)) if u>=16]:
        for numUnit3 in [u for u in (numUnit2>>j for j in range(10)) if u>=16]:
            try:
                ...
                if os.path.exists('results_GAT_2GCN_2FFNN_64_8_elu.json'):
                    with open('results_GAT_2GCN_2FFNN_64_8_elu.json', 'r') as f:
                        results = json.load(f)
                else:
                    results = []
                results.append(r)
                with open('results_GAT_2GCN_2FFNN_64_8_elu.json', 'w') as f:
