        ...
        for numUnit3 in [u for u in (numUnit2>>j for j in range(10)) if u>=16]:
            try:
                ...
                for train_idx, val_idx in kf.split(dataset):
                    ...
                    #torch.save(model.state_dict(), f"models/model_GAT_1GCN_2FFNN_elu_{cnt}_{hidden_dim}_{heads}_{numUnit1}_{numUnit2}_{numUnit3}.pt")
                    cnt += 1
                ...
                if os.path.exists('results_GAT_1GCN_2FFNN_elu_64_8.json'):
                    with open('results_GAT_1GCN_2FFNN_elu_64_8.json', 'r') as f:
                        results = json.load(f)
                else:
                    results = []
                results.append(r)
                with open('results_GAT_1GCN_2FFNN_elu_64_8.json', 'w') as f:
