...
for fileName in listFile:
    if fileName.endswith('.qasm'):
        ...
        try:
            ...
            with open('log-Montreal.txt', 'a') as log_file:
        ...
        for opt in range(4):
            try:
                ...
                continue
        pd.DataFrame.from_dict(resDict, orient='index').to_csv('IBM-Montreal-res.csv')
    
                #print("\nTranspiled Circuit:")

            #print(f"Probability of measuring {target_state}: {probability:.4f}")

pd.DataFrame.from_dict(resDict, orient='index').to_csv('results-Montreal.csv')
