...
dataset = torch.load('montreal.pt', weights_only=False)
        ...
        for numUnit3 in [u for u in (numUnit2>>j for j in range(10)) if u>=16]:
            try:
                ...
                for train_idx, val_idx in kf.split(dataset):
                    ...
                    #torch.save(model.state_dict(), f"models/model_GAT_2GCN_3FFNN_elu_{cnt}_{hidden_dim}_{heads}_{numUnit1}_{numUnit2}_{numUnit3}.pt")
                    cnt += 1
                ...
                if os.path.exists('results_GAT_2GCN_3FFNN_elu.json'):
                    with open('results_GAT_2GCN_3FFNN_elu.json', 'r') as f:
                        results = json.load(f)
                else:
                    results = []
                results.append(r)
                with open('results_GAT_2GCN_3FFNN_elu.json', 'w') as f:
