      ...
      for numUnit2 in [u for u in (numUnit1>>i for i in range(10)) if u>=16]:
        for numUnit3 in [u for u in (numUnit2>>j for j in range(10)) if u>=16]:
            try:
                ...
                if os.path.exists('results_GAT_2GCN_3FFNN_sigmoid.json'):
                    with open('results_GAT_2GCN_3FFNN_sigmoid.json', 'r') as f:
                        results = json.load(f)
                else:
                    results = []
                results.append(r)
                with open('results_GAT_2GCN_3FFNN_sigmoid.json', 'w') as f:
